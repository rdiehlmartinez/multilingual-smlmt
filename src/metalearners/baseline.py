...
class BaselineLearner(BaseLearner):
    def __init__(self, **kwargs) -> None:
        ...
        assert (
            self.retain_lm_head is True
        ), "BaselineLearner requires retain_lm_head to be True"
    ...
    def run_train_loop(
        ...
        for data_batch in support_batch_list + [query_batch]:
            ...
            _, loss = self._compute_task_loss(
                outputs, data_batch, self.retained_lm_head_weights, "classification"
            )
            ...
            for param, grad in zip(
                [p for p in self.base_model.parameters() if p.requires_grad], grads
            ):
