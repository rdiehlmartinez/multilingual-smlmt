...
def classification_random(
    ...
    classifier_weight = (
        torch.randn((n_labels, base_model_hidden_dim), device=device) * std_weight
    )
...
def classification_random_fc(
    base_model_hidden_dim: int, n_labels: int, **kwargs: Dict[str, Any]
) -> nn.ParameterDict:
    """
    Initializes classification task head using a random Xavier-He initialization method.
    ...
    fc_head_weights = classification_random(
        base_model_hidden_dim, base_model_hidden_dim, **kwargs
    )
    classifier_weights = classification_random(
        base_model_hidden_dim, n_labels, **kwargs
    )
