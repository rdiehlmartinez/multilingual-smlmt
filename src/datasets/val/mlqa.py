...
class MLQAGenerator(NLUTaskGenerator): 
    ...
    def __init__(self, config: ConfigParser, eval_type: str) -> None:
        ...
        self.train_data_dir = config.get(self.config_name, "train_data_dir")
        self.dev_data_dir = config.get(self.config_name, "dev_data_dir")
        self.eval_data_dir = config.get(self.config_name, "eval_data_dir")
    ...
    def run_evaluation(
        ...
        **kwargs
        ...
        features, _ = features_and_dataset["features"], features_and_dataset["dataset"]
        ...
        for split_batch in split_dataloader:
            ...
            with torch.no_grad():
                ...
                total_loss += split_loss.item() * batch_size # loss averaged across the batch
            ...
            for i, example_index in enumerate(split_example_indices):
                ...
                result = SquadResult(unique_id, example_start_logits, example_end_logits)
...
def main():
    ...
    config = ConfigParser()
    config.add_section("MLQA_STANDARD")
    config.set(
        "MLQA_STANDARD",
        "train_data_dir",
        "../../rds-personal-3CBQLhZjXbU/data/xtreme/download/squad",
    )

    config.set(
        "MLQA_STANDARD",
        "dev_data_dir",
        "../../rds-personal-3CBQLhZjXbU/data/xtreme/download/mlqa/MLQA_V1/dev",
    )

    config.set(
        "MLQA_STANDARD",
        "eval_data_dir",
        "../../rds-personal-3CBQLhZjXbU/data/xtreme/download/mlqa/MLQA_V1/test",
    )

    config.set("MLQA_STANDARD", "eval_languages", "en, de")


    config.set("MLQA_STANDARD", "batch_size", "16")

    config.set("MLQA_STANDARD", "max_epochs", "16")

    config.set("MLQA_STANDARD", "lr", "1e-5")

    config.set("MLQA_STANDARD", "task_head_init_method", "random")

    mlqa_generator = MLQAGenerator(config, "standard")

    for data_dict in mlqa_generator:
        pass
