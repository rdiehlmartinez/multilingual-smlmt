...
class NLUTaskGenerator(metaclass=abc.ABCMeta):
    def __init__(self, eval_type: str) -> None:
        ...
        self._batch_size = wandb.config[self.config_name]["batch_size"]
        self._max_epochs = wandb.config[self.config_name]["max_epochs"]
        self._lr = wandb.config[self.config_name]["lr"]
        self._task_head_init_method = wandb.config[self.config_name][
        ...
        self.eval_languages = wandb.config[self.config_name][
            "eval_languages"
        ].split(",")
    ...
    def run_finetune_evaluation(
        ...
        Tuple[float, float], Tuple[float, float, Dict[str, List[float]]]
    ]:
        """
        ...
        * finetune_info: A dictionary containing the losses and accuracies for the finetuning
        ...
        finetune_optimizer_param_groups = (
            learner.finetune_optimizer_param_groups(
                finetune_model,
                task_head_weights,
        ...
        finetune_optimizer = AdamW(finetune_optimizer_param_groups, lr=self.lr)
        ...
        for epoch in range(self.max_epochs):
            ...
            for finetune_batch in finetune_dataloader:
                ...
                if (
                    ...
                    if best_dev_metric is None or self.metric_is_better(
                        dev_result["metric"], best_dev_metric
                    ):
        ...
        if self.eval_type == "cross_lingual":
            eval_results["finetune_language"] = task_data["finetune"][
                "language"
            ]
    ...
    def get_eval_task_head_init_kwargs(
        ...
        init_kwargs["device"] = (
            device if device is not None else learner.base_device
        )
