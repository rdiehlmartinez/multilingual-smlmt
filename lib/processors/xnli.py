...
class XnliProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, language="en", split="train"):
        ...
        for lg in language.split(","):
            ...
            for i, line in enumerate(lines):
                ...
                text_b = line[1]
                if len(line) != 3:
                    # NOTE: This should not happen in the current version
                    raise Exception("Data format error: %s" % line)
    ...
    def get_translate_train_examples(self, data_dir, language="en"):
        ...
        for lg in language.split(","):
            file_path = os.path.join(
                data_dir,
                "translate-train/XNLI_translate-train_en-{}-translated.tsv".format(
                    lg
                ),
            )
    ...
    def get_translate_test_examples(self, data_dir, language="en"):
        raise NotImplementedError("DOUBLE CHECK THE DATA FOLDER PATH")
        lg = language
        lines = self._read_tsv(
            os.path.join(
                data_dir,
                "XNLI-Translated/test-{}-en-translated.tsv".format(lg),
            )
        )
        examples = []
        for i, line in enumerate(lines):
